...
---Create a block separator renderer function
---Produces a modern 2-line virtual divider with block accent
---@return fun(ctx: table): table virt_lines
local function create_block_renderer()
	return function(ctx)
		...
		local rule_len = math.max(24, 2 + title_w + badge_w)
		local rule_chunks = {
			{ "▀" .. string.rep("─", rule_len - 1), "AgyDividerLine" },
		}
...
return setmetatable(M, {
	__call = function(_, ctx_or_opts)
		if type(ctx_or_opts) == "table" and ctx_or_opts.role ~= nil then
			return create_block_renderer()(ctx_or_opts)
		end
		return create_block_renderer()
	end,
})
